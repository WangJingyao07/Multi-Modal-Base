...
def model_forward(i_epoch, model, args, criterion, batch):
    txt, segment_t, mask_t, visual, audio, tgt, idx = batch
    txt, mask_t, segment_t, audio,visual, tgt = txt.cuda(), mask_t.cuda(), segment_t.cuda(), audio.cuda(),visual.cuda(), tgt.cuda()
    fusion_out, a_logits, t_logits, v_logits, adap_a, adap_t, adap_v = model(txt, mask_t, segment_t, audio, visual)


    audio_loss = nn.CrossEntropyLoss()(a_logits, tgt)
    txt_loss = nn.CrossEntropyLoss()(t_logits, tgt)
    visual_loss = nn.CrossEntropyLoss()(v_logits, tgt)
    
    adap_a_loss = nn.CrossEntropyLoss()(adap_a, tgt)
    adap_t_loss = nn.CrossEntropyLoss()(adap_t, tgt)
    adap_v_loss = nn.CrossEntropyLoss()(adap_v, tgt)

    # 2)0.6 0.6 0.6  3)0.6 0.9 0.7   4)0.1 0.9 0.2  5)0.5 0.5 0.5
    loss = audio_loss + txt_loss + visual_loss + args.alpha1 * adap_a_loss + args.alpha2 * adap_t_loss + args.alpha3 * adap_v_loss

    return loss, fusion_out, a_logits, t_logits, v_logits, tgt
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        for batch in data:
            loss, fusion_out, a_logits, t_logits, v_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
            losses.append(loss.item())

        
            pred = torch.nn.functional.softmax(fusion_out, dim=1).argmax(dim=1).cpu().detach().numpy()
            pred_audio = torch.nn.functional.softmax(a_logits, dim=1).argmax(dim=1).cpu().detach().numpy()
            pred_txt = torch.nn.functional.softmax(t_logits, dim=1).argmax(dim=1).cpu().detach().numpy()
            pred_visual = torch.nn.functional.softmax(v_logits, dim=1).argmax(dim=1).cpu().detach().numpy()
...
def train(args):
    ...
    for i_epoch in range(start_epoch, args.max_epochs):
        ...
        # writer.add_scalar('Accuracy/val_txt', metrics["acc_txt"], i_epoch)
