...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        for batch in data:
            ...
            loss, txt_img_logits, txt_logits, img_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
...
#     # l_mi_txt = torch.mean(torch.linalg.vector_norm(txt_logits+noise.cuda(), ord=2, dim=1))
#     # l_mi_img = torch.mean(torch.linalg.vector_norm(img_logits+noise.cuda(), ord=2, dim=1))

#     # l_mi = l_mi_mix + l_mi_txt + l_mi_img
#     l_mi = l_mi_mix
...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    logits_a, mu_a, logvar_a, logits_v, mu_v, logvar_v = model(txt, mask, segment, img)

    # logits, mu, logvar = model(spec, visual)
    kl1 = -0.5 * torch.sum(1 + logvar_a - mu_a.pow(2) - logvar_a.exp())

    
    kl2 = -0.5 * torch.sum(1 + logvar_v - mu_v.pow(2) - logvar_v.exp())
    kl12 = -0.5 * torch.sum(1 + (logvar_a - logvar_v) - ((mu_a - mu_v).pow(2) + logvar_a.exp()) / logvar_v.exp())
    ...
    # loss = audio_visual_loss + audio_clf_loss + visaul_loss + 0.001*(kl1 + kl2+ kl12)
    loss = audio_visual_loss + audio_clf_loss + visaul_loss 
...
def train(args):
    ...
    for i_epoch in range(start_epoch, args.max_epochs):
        ...
        for batch in tqdm(train_loader, total=len(train_loader)):
            loss, a_v_logits, logits_a, logits_v, tgt = model_forward(i_epoch, model, args, criterion, batch)
