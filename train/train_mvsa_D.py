...
def get_args(parser):
    ...
    parser.add_argument("--model", type=str, default="mml_vt", choices=["mutual","mml_vt", "latefusion"])
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        for batch in data:
            ...
            fusion_logits, txt_clf_loss, img_clf_loss, txt_logits, img_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
            losses.append(txt_clf_loss.item())
...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    return fusion_logits, txt_clf_loss, img_clf_loss, txt_logits, img_logits, tgt
...
def train(args):
    ...
    for i_epoch in range(start_epoch, args.max_epochs):
        ...
        for batch in tqdm(train_loader, total=len(train_loader)):
            ...
            # # txt 蒸馏 visual
            # pred_txt = nn.functional.softmax(txt_logits, dim=1).argmax(dim=1)
            # loss_img = nn.CrossEntropyLoss()(img_logits, pred_txt)
            # loss = txt_clf_loss + loss_img + img_clf_loss


            # soft zhengliu
            # img_log = nn.functional.log_softmax(img_logits, dim=1)
            # target_txt = nn.functional.softmax(txt_logits, dim=1)
            # loss_soft_img = nn.KLDivLoss(reduction='batchmean')(img_log, target_txt)
            # loss = txt_clf_loss + loss_soft_img + img_clf_loss
            # loss = loss_soft_img + txt_clf_loss



            # visual 蒸馏 txt
            pred_img = nn.functional.softmax(img_logits, dim=1).argmax(dim=1)
            loss_txt = nn.CrossEntropyLoss()(txt_logits, pred_img)
            loss =  img_clf_loss + loss_txt + txt_clf_loss

            # soft zhengliu 
            # txt_log = nn.functional.log_softmax(txt_logits, dim=1)
            # target_img = nn.functional.softmax(img_logits, dim=1)
            # loss_soft_txt = nn.KLDivLoss(reduction='batchmean')(txt_log, target_img)
            # # loss = img_clf_loss + loss_soft_txt + txt_clf_loss
            # loss = loss_soft_txt + img_clf_loss
