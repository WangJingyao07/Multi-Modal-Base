...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        for batch in data:
            txt_clf_loss, img_clf_loss, txt_logits, img_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
            loss = txt_clf_loss 
            # loss =  img_clf_loss
...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    return txt_clf_loss, img_clf_loss, txt_logits, img_logits, tgt
