...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    a, v = model(spec, visual)
    audio_logits = model.mix_head(a)
    visual_logits = model.mix_head(v)
    a_v_logits = model.mix_head((a+v)*0.5)

    audio_clf_loss = nn.CrossEntropyLoss()(audio_logits, tgt)
    visaul_loss = nn.CrossEntropyLoss()(visual_logits, tgt)
    a_vloss = nn.CrossEntropyLoss()(a_v_logits, tgt)
    # audio_visual_logits = 0.5*audio_logits + 0.5*visual_logits
    noise = torch.randn(a_v_logits.shape) * np.sqrt(1.0 / 2 * np.pi)
    l_mi_mix = torch.mean(torch.linalg.vector_norm(a_v_logits+noise.cuda(), ord=2, dim=1))

    # loss = audio_clf_loss + visaul_loss
    # loss = audio_clf_loss + visaul_loss + l_mi_mix
    loss = a_vloss + l_mi_mix
    return loss, a_v_logits, audio_logits, visual_logits, tgt
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        # accuracy_2 = 0
        for batch in data:
            # loss, audio_logits, visual_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
            loss, a_v_logits, audio_logits, visual_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
...
# 
            ...
            pred = nn.functional.softmax((a_v_logits), dim=1).argmax(dim=1).cpu().detach().numpy()
