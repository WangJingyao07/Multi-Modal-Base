...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    audio_logits, visual_logits = model(spec, visual)

    audio_clf_loss = nn.CrossEntropyLoss()(audio_logits, tgt)
    visaul_loss = nn.CrossEntropyLoss()(visual_logits, tgt)

    return audio_clf_loss, visaul_loss, audio_logits, visual_logits, tgt
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        # accuracy_2 = 0
        for batch in data:
            audio_clf_loss, visaul_loss, audio_logits, visual_logits, tgt= model_forward(i_epoch, model, args, criterion, batch)
            # loss = audio_clf_loss
            loss = visaul_loss
