...
def get_args(parser):
    ...
    parser.add_argument("--model", type=str, default="mml_vt", choices=["mutual","mml_avt", "latefusion"])
...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    fu_logits, logits_t, logits_i = model(txt, mask, segment, img)
    
    
    audio_clf_loss = nn.CrossEntropyLoss()(logits_t, tgt)
    visaul_loss = nn.CrossEntropyLoss()(logits_i, tgt)
    audio_visual_loss = nn.CrossEntropyLoss()((logits_t+logits_i)/2, tgt)
    loss = audio_visual_loss 

    return loss, (logits_i+logits_t)/2, logits_t, logits_i, tgt
