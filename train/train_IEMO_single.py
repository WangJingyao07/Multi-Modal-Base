...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    fusion, audio_logits, visual_logits, txt_logits, embeding_loss = model(txt, mask_t, segment_t, audio, visual)
    ...
    return loss, fusion, audio_logits, visual_logits, txt_logits, tgt
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        for batch in data:
            loss,fusion, audio_logits, visual_logits, txt_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
