...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    # if args.mask == "soft_mask": # soft mask
    #     audio_visual_logits, audio_logits, visual_logits, mask = model(spec, visual)
    #     l1_reg = torch.norm(mask, p=1)
    #     noise = torch.randn(audio_visual_logits.shape) * np.sqrt(1.0 / 2 * np.pi)
    #     l_mi_mix = torch.mean(torch.linalg.vector_norm(audio_visual_logits+noise.cuda(), ord=2, dim=1))
    #     l_mi = l_mi_mix


    #     audio_clf_loss = nn.CrossEntropyLoss()(audio_logits, tgt)
    #     visaul_loss = nn.CrossEntropyLoss()(visual_logits, tgt)
    #     audio_visual_loss = nn.CrossEntropyLoss()(audio_visual_logits, tgt)

    #     loss = audio_visual_loss + audio_clf_loss + visaul_loss + l_mi + l1_reg

    # else:
    # audio_visual_logits, audio_logits, visual_logits, soft_mask1, soft_mask2 = model(spec, visual)

    ## main code
    audio_visual_logits, audio_logits, visual_logits  = model(spec, visual)
    noise = torch.randn(audio_visual_logits.shape) * np.sqrt(1.0 / 2 * np.pi)
    l_mi_mix = torch.mean(torch.linalg.vector_norm(audio_visual_logits+noise.cuda(), ord=2, dim=1))
    l_mi = l_mi_mix


    # l1_reg = torch.norm(soft_mask1, p=1) + torch.norm(soft_mask2, p=1)
    ...
    return loss, audio_visual_logits, audio_logits, visual_logits, tgt
