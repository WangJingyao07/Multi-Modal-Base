...
def get_args(parser):
    ...
    parser.add_argument('--model', default='mml_avt', type=str, choices=['mml_vt','mml_avt', 'latefusion', 'tmc'])
...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    fusion, audio_logits, visual_logits, txt_logits = model(txt, mask_t, segment_t, audio, visual)

    fusion_loss = nn.CrossEntropyLoss()(fusion, tgt)
    audio_clf_loss = nn.CrossEntropyLoss()(audio_logits, tgt)
    visaul_loss = nn.CrossEntropyLoss()(visual_logits, tgt)
    txt_loss = nn.CrossEntropyLoss()(txt_logits, tgt)
   
    
    return fusion, fusion_loss, audio_clf_loss, visaul_loss, txt_loss, audio_logits, visual_logits, txt_logits, tgt
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        for batch in data:
#
            fusion, fusion_loss, audio_clf_loss, visaul_loss, txt_loss, audio_logits, visual_logits, txt_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
            losses.append(fusion_loss.item())

        
            pred = torch.nn.functional.softmax(fusion, dim=1).argmax(dim=1).cpu().detach().numpy()
...
def train(args):
    ...
    for i_epoch in range(start_epoch, args.max_epochs):
        ...
        for batch in tqdm(train_loader, total=len(train_loader)):
            ...
            loss_value_mm, loss_value_v, loss_value_a, loss_value_t = [], [], [], []
            cos_audio, cos_visual, cos_txt = [], [], []
            record_names_audio, record_names_visual, record_names_txt  = [], [], []
            ...
            loss_value_a.append(audio_clf_loss.item())
            loss_value_v.append(visaul_loss.item())
            loss_value_t.append(txt_loss.item())


            losses=[fusion_loss, audio_clf_loss, visaul_loss, txt_loss]
            all_loss = ['both', 'audio', 'visual', 'txt']
            ...
            loss = fusion_loss + audio_clf_loss + visaul_loss + txt_loss
            loss.backward()
