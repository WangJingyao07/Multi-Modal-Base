...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    a, v, t = model(txt, mask_t, segment_t, audio, visual)
    ...
    loss = loss_a + loss_v + loss_t

      
    return loss, audio_logits, visual_logits, txt_logits, tgt


    # noise = torch.randn(m3fusion_logits.shape) * np.sqrt(1.0 / 2 * np.pi)
    # l_mi_mix = torch.mean(torch.linalg.vector_norm(m3fusion_logits+noise.cuda(), ord=2, dim=1))
    # l_mi = l_mi_mix
    ...
    # m3fusion_clf_loss = nn.CrossEntropyLoss()(m3fusion_logits, tgt)
   
    # loss = m3fusion_clf_loss + audio_clf_loss + visaul_loss + txt_loss + l_mi
    # # loss = m3fusion_clf_loss + audio_clf_loss + visaul_loss + txt_loss  


    # return loss, m3fusion_logits, audio_logits, visual_logits, txt_logits, tgt
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        for batch in data:
            loss, audio_logits, visual_logits, txt_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
          
    
            losses.append(loss.item())

            m3fusion_logits = (audio_logits + visual_logits + txt_logits) / 3
            pred = torch.nn.functional.softmax(m3fusion_logits, dim=1).argmax(dim=1).cpu().detach().numpy()
...
# 
    ...
    for i_epoch in range(start_epoch, args.max_epochs):
        ...
        for batch in tqdm(train_loader, total=len(train_loader)):
            ...
            #     loss_v = loss_v / args.gradient_accumulation_steps
