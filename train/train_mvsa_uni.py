...
def get_args(parser):
    ...
    parser.add_argument("--data_path", type=str, default="/data/users/wjy/data/Robumm/datasets")
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        for batch in data:
            ...
            ti_loss, txt_clf_loss, img_clf_loss, txt_logits, img_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
            losses.append(ti_loss.item())

        
            pred = torch.nn.functional.softmax(0.5*(txt_logits+img_logits), dim=1).argmax(dim=1).cpu().detach().numpy()
...
def model_forward(i_epoch, model, args, criterion, batch):
    # 假设 batch 包含：txt, segment, mask, img, tgt, idx
    txt, segment, mask, img, tgt, idx = batch
    txt, mask, segment, img, tgt = txt.cuda(), mask.cuda(), segment.cuda(), img.cuda(), tgt.cuda()

    # 模型返回文本和图像 logits
    txt_logits, img_logits = model(txt, mask, segment, img)
    # 调用混合头进行映射（如果需要，可在模型中定义）：
    txt_logits = model.mix_head(txt_logits)
    img_logits = model.mix_head(img_logits)

    # 融合 logits（最终预测使用融合 logits）
    fusion_logits = 0.5 * (txt_logits + img_logits)
    ...
    # 蒸馏损失：让图像 logits 学习文本 logits 的分布
    # 使用温度系数 softening
    T = 0.5
    # # 计算 soft targets：文本 logits作为教师
    txt_p = nn.functional.softmax(txt_logits / T, dim=1)
    # # 学生：图像 logits
    img_p = nn.functional.softmax(img_logits / T, dim=1)
    fusion_p = 0.5 * (img_p + txt_p)
    # kd_loss = nn.functional.kl_div(log_student, soft_targets, reduction="batchmean") * (T * T)
    kl1 = 0.5* nn.functional.kl_div(img_p.log(), fusion_p, reduction="batchmean") * (T * T)
    kl2 = 0.5* nn.functional.kl_div(txt_p.log(), fusion_p, reduction="batchmean") * (T * T)
    jsd_loss = kl1 + kl2
    
    # 总损失：融合预测为主，同时加上单模态损失和蒸馏损失
    loss =  fusion_loss +  txt_clf_loss + img_clf_loss +  jsd_loss

    return loss, txt_clf_loss, img_clf_loss, txt_logits, img_logits, tgt
...
def train(args):
    ...
    for i_epoch in range(start_epoch, args.max_epochs):
        ...
        for batch in tqdm(train_loader, total=len(train_loader)):
            loss, txt_clf_loss, img_clf_loss, txt_logits, img_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
