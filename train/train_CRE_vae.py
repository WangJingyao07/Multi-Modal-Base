...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    # logits, mu, logvar = model(spec, visual)
    # kl1 = -0.5 * torch.sum(1 + logvar_a - mu_a.pow(2) - logvar_a.exp(), dim=1).mean()
    # kl2 = -0.5 * torch.sum(1 + logvar_v - mu_v.pow(2) - logvar_v.exp(), dim=1).mean()
    # kl12 = -0.5 * torch.sum(
    #         1 + logvar_a - logvar_v - ((mu_a - mu_v).pow(2) + logvar_v.exp()) / logvar_a.exp(), dim=1
    #     ).mean()
    
### met
    # logits_av, mu_a, logvar_a, mu_v, logvar_v = model(spec, visual)
    # a_v_clf_loss = nn.CrossEntropyLoss()(logits_av, tgt)
    # # kl_a = -0.5 * torch.sum(1 + logvar_a - mu_a.pow(2) - logvar_a.exp())
    # # kl_v = -0.5 * torch.sum(1 + logvar_v - mu_v.pow(2) - logvar_v.exp())

    # kl_a = 0.5 * torch.sum(mu_a.pow(2) + logvar_a.exp() -1 - logvar_a)
    # kl_v = 0.5 * torch.sum(mu_v.pow(2) + logvar_v.exp() -1 - logvar_v)
    # kl = kl_a + kl_v
    # kl_bit = kl / math.log(2)
    # kl_mean = kl_bit.mean()
    # loss = a_v_clf_loss + 0.01*kl_mean
    # return loss, logits_av, tgt

    logits_a, mu_a, logvar_a, logits_v, mu_v, logvar_v = model(spec, visual)
    # kl1 = -0.5 * torch.sum(1 + logvar_a - mu_a.pow(2) - logvar_a.exp(), dim=1).mean()
    # kl2 = -0.5 * torch.sum(1 + logvar_v - mu_v.pow(2) - logvar_v.exp(), dim=1).mean()
    # kl12 = -0.5 * torch.sum( 1 + logvar_a - logvar_v - ((mu_a - mu_v).pow(2) + logvar_v.exp()) / logvar_a.exp(), dim=1
    #     ).mean()
#     kl12 = 0.5 * torch.sum(
#     logvar_v - logvar_a + ((mu_a - mu_v).pow(2) + logvar_a.exp()) / logvar_v.exp() - 1, dim=1
# ).mean()

    kl_weight = min(0.001 * i_epoch, 1.0)
    audio_clf_loss = nn.CrossEntropyLoss()(logits_a, tgt)
    visaul_loss = nn.CrossEntropyLoss()(logits_v, tgt)
    # loss = audio_clf_loss + visaul_loss
    a_v_logits = (logits_a + logits_v) / 2
    T = 0.1
    audio_visual_loss = nn.CrossEntropyLoss()(a_v_logits, tgt)
    loss1 = nn.functional.kl_div(nn.functional.softmax(logits_a / T, dim=1),
                         nn.functional.softmax(logits_v / T, dim=1),
                         reduction="batchmean")
    # loss = audio_visual_loss + audio_clf_loss + visaul_loss + kl_mean # not IB
    # loss = audio_visual_loss + audio_clf_loss + visaul_loss + l_mi
    # loss = audio_visual_loss + audio_clf_loss + visaul_loss +0.01*(kl1 + kl2 + kl12)
    # kl_loss = kl1 + kl2 
    loss = audio_visual_loss + audio_clf_loss + visaul_loss +  loss1
    # print("kl1 and kl2 and kl12", kl1, kl2, kl12)
    # print("kl1 and kl2 and kl12", kl1.item(), kl2.item(), kl12.item())
    # loss = audio_clf_loss + visaul_loss 
    return loss, a_v_logits, logits_a, logits_v, tgt
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        # accuracy_2 = 0
        for batch in data:
            loss, audio_visual_logits, audio_logits, visual_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
            # loss, audio_visual_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
            losses.append(loss.item())
        
            pred = nn.functional.softmax(audio_visual_logits, dim=1).argmax(dim=1).cpu().detach().numpy()
...
def train(args):
    ...
    for i_epoch in range(start_epoch, args.max_epochs):
        ...
        for batch in tqdm(train_loader, total=len(train_loader)):
            # loss, a_v_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
            loss, audio_visual_logits, audio_logits, visual_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
