...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    m3fusion_logits, audio_logits, visual_logits, txt_logits = model(txt, mask_t, segment_t, audio, visual)
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    with torch.no_grad():
        ...
        for batch in data:
            loss, m3fusion_logits, audio_logits, visual_logits, txt_logits, tgt = model_forward(i_epoch, model, args, criterion, batch)
