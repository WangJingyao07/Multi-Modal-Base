...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, modality, num_classes=1000, pool='avgpool', zero_init_residual=False,
        ...
        self.layer5 = self._make_layer(block, 768, layers[4])
...
def resnet18(modality, progress=True, **kwargs):
    return _resnet('resnet18', BasicBlock, [2, 2, 2, 2, 2], modality, progress,
                   **kwargs)


import torch

def test_resnet18(modality):
    net = resnet18(modality)

    if modality == 'audio':

        input_tensor = torch.randn(6, 1, 257, 188)
    elif modality == 'visual':

        input_tensor = torch.randn(6, 3, 10, 224, 224) #[B, channel, sampels,H,W]
    else:
        raise ValueError("Unsupported modality. Choose 'audio' or 'visual'.")
