...
def load_checkpoint(model, path):
    best_checkpoint = torch.load(path, weights_only=True)
...
def log_metrics(set_name, metrics, args, logger):
    if args.task_type == "multilabel":
        ...
        if args.dataset == "CREMAD":
            logger.info(
                "{}: Loss: {:.5f} | Acc: {:.5f} | Acc_audio: {:.5f} | Acc_visual: {:.5f} | f1_score: {:.5f}".format(
                    set_name, metrics["loss"], metrics["acc"], metrics["acc_audio"], metrics["acc_visual"], metrics["micro_f1"]
                )
            )
        elif args.dataset == "IEMOCAP":
            logger.info(
                "{}: Loss: {:.5f} | Acc: {:.5f} | Acc_audio: {:.5f} | Acc_visual: {:.5f} | Acc_txt: {:.5f} | f1_score: {:.5f}".format(
                    set_name, metrics["loss"], metrics["acc"], metrics["acc_audio"], metrics["acc_visual"] , metrics["acc_txt"] ,metrics["micro_f1"]  #
                )
            )
        else:
            logger.info(
            "{}: Loss: {:.5f} | Acc: {:.5f} | Acc_txt: {:.5f} | Acc_img: {:.5f} | f1_score: {:.5f}".format(
                set_name, metrics["loss"], metrics["acc"], metrics["acc_txt"], metrics["acc_img"], metrics["micro_f1"]
            )
        )
